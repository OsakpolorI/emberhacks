...
try {
  if (process.argv.includes('--list')) {
    ...
    for await (const model of models) {
      if (/flash|live/.test(model.name ?? ''))
        console.log(JSON.stringify({ name: model.name, actions: model.supportedActions }));
    }
    process.exit(0);
  }
  if (!process.argv.includes('--live-only')) {
    const text = await ai.models.generateContent({
      ...
      config: { maxOutputTokens: 30, httpOptions: { timeout: 20000 } },
    });
    console.log(
      JSON.stringify({
        check: 'text inference',
        model: process.env.GEMINI_ANALYSIS_MODEL,
        success: !!text.text,
      }),
    );
  ...
  await new Promise(async (resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      session?.close();
      reject(new Error('Live audio response timed out'));
    }, 20000);
    try {
      session = await ephemeral.live.connect({
        ...
        callbacks: {
          onmessage: (message) => {
            if (message.serverContent?.modelTurn?.parts?.some((p) => p.inlineData?.data)) {
              done = true;
              clearTimeout(timeout);
              console.log(
                JSON.stringify({
                  check: 'live audio response',
                  model: process.env.GEMINI_LIVE_MODEL,
                  success: true,
                }),
              );
          ...
          onclose: (event) => {
            if (!done) {
              ...
              clearTimeout(timeout);
              reject(new Error(`Live closed: ${event.reason}`));
            }
          },
        },
      });
  ...
  console.error(String(error.message ?? error).replace(/AIza[\w-]+/g, '[REDACTED]'));
  process.exitCode = 1;
}
